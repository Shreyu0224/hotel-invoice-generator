...
// ---------- modal open/close ----------
var subModalMandatory = false; // tracks whether the currently-open modal can be dismissed without choosing a plan

function openSubscriptionModal(opts){
  opts = opts || {};
  subModalMandatory = !!opts.mandatory;
  ...
  if(closeBtn){
    ...
    closeBtn.title = subModalMandatory ? 'Log out' : 'Close';
    closeBtn.setAttribute('aria-label', subModalMandatory ? 'Log out' : 'Close');
  }
...
// Called by the X button. If the plan choice was mandatory (no active
// subscription yet, or it just expired), there's nothing to safely show
// behind the modal — so instead of just closing it, sign the admin out
// and return them to the login screen. Otherwise it's a normal close.
function handleSubscriptionCloseClick(){
  if(subModalMandatory){
...
(function(){
  document.getElementById('subscriptionCloseBtn').addEventListener('click', handleSubscriptionCloseClick);
...
function showSubscriptionResult(success, title, message){
  ...
  // The X stays available even on failure/cancel — for a mandatory flow
  // it now logs the admin out (handleSubscriptionCloseClick) rather than
  // trapping them with no way out except retrying payment.
